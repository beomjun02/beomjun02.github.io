...
- **One log ≈ 1–2 weeks on one question, not one per step.** Default to UPDATING the project's open (`running`) log — dated Timeline entry, refreshed verdict, bumped `updated` in `logs.json` — rather than creating another log.
